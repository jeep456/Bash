...
sqlNeedSeries = "SELECT seria FROM need_series;"
selectFromBaseNeed = subprocess.check_output(['mysql','-N', '-uroot', '-p123', 'lars', '-e', sqlNeedSeries]).decode(encoding='utf-8').strip()
selectFromBaseNeed = selectFromBaseNeed.split('\n')
# selectFromBaseNeed = tuple(selectFromBaseNeed)

# print(novinki[3][0] in selectFromBaseNeed)



# print(type(selectFromBaseNeed))


def insertToBase (title,episod):
    sqlInsert = "INSERT INTO `series` VALUES (NULL,'" + title + "','" + episod + "','" + time.strftime("%Y-%m-%d") + "','" + time.strftime("%Y-%m-%d %H:%M:%S") + "','" + time.strftime("%Y-%m-%d %H:%M:%S") + "');"
...
if len(seriaCheckBase(novinki[0])) == 0 :
    ...
    print("Пишем в базу - " + time.strftime("%Y-%m-%d %H:%M:%S"))
    ...
    print("Пока новинок нет - " + time.strftime("%Y-%m-%d %H:%M:%S"))
